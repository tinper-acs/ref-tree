...
defaultExpandAll |`bool`|-- | 展开所有节点，true 展开，false 不展开。懒加载下默认为false,defaultExpandAll不起效。| 否
...
value| ``string``| 空 |带有input框参照的input默认值。格式必须符合`'{"refname":"初级-T1","refpk":"level1"}'`。refname字段不可变，refpk指定的值是数据的键对应的值。|否
