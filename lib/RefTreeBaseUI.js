...
function _defaults(obj, defaults) { var keys = (0, _getOwnPropertyNames2["default"])(defaults); for (var i = 0; i < keys.length; i++) { var key = keys[i]; var value = (0, _getOwnPropertyDescriptor2["default"])(defaults, key); if (value && value.configurable && obj[key] === undefined) { (0, _defineProperty3["default"])(obj, key, value); } } return obj; }
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.initComponent = function (props) {
    ...
    if (!valueMap[valueField] && props.value !== _this4.props.value) {
      _this4.setState({
        checkedArray: [],
        selectedArray: [],
        showLoading: false,
    ...
    if (matchData.length > 0) {
      ...
      //数据多个
      var values = [];
      var names = valueMap.refname.split(',');
      valueMap[valueField].split(',').forEach(function (item, i) {
        if (!item) return;
        values.push(_defineProperty({ refname: names[i] }, valueField, item));
      });
      _this4.setState({
        checkedArray: values,
